                                ...
                                EGLConfig* result)
{
    ...
    if (ctxconfig->client == GLFW_OPENGL_ES_API || (_glfw.platform.platformID == GLFW_PLATFORM_ANDROID))
    {
                               ...
                               const _GLFWfbconfig* fbconfig)
{
    ...
    if (_glfw.egl.KHR_create_context)
    {
        ...
        {
            int major = ctxconfig->major;
            int minor = ctxconfig->minor;
            if (_glfw.platform.platformID == GLFW_PLATFORM_ANDROID && ctxconfig->client == GLFW_OPENGL_API)
            {
                if (mojoexec_renderspec.override_major_version > 0)
                    major = mojoexec_renderspec.override_major_version;
                else if (major < 3)
                    major = 3;
            }
            if (major != 1 || minor != 0)
            {
                SET_ATTRIB(EGL_CONTEXT_MAJOR_VERSION_KHR, major);
                SET_ATTRIB(EGL_CONTEXT_MINOR_VERSION_KHR, minor);
            }
        }
    ...
    else
    {
        if (ctxconfig->client == GLFW_OPENGL_ES_API || (_glfw.platform.platformID == GLFW_PLATFORM_ANDROID))
        {
            ...
            SET_ATTRIB(EGL_CONTEXT_CLIENT_VERSION, major);
        }
    }
